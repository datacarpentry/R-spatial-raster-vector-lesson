...
> > ```
> > 3) We can use the same functions to figure out the differences:
> > ```{r}
> > Layer_Functions<-attr(methods(class=class(RGB_stack_HARV[[1]])),'info')$generic
> > Stack_Functions<-attr(methods(class=class(RGB_stack_HARV)),'info')$generic
> > Stack_Functions[!(Stack_Functions%in%Layer_Functions)]
> > Layer_Functions[!(Layer_Functions%in%Stack_Functions)]
> > ```
> {: .solution}
{: .challenge}
